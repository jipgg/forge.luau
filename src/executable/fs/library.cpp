...
static auto subpaths(auto L) -> int {
    ...
    return wow::fs::push_directory_iterator(L, directory, recursive);
}
static auto currdir(auto L) -> int {
    return wow::fs::push_path(L, std::filesystem::current_path());
}
...
static auto tmpdir(auto L) -> int {
    ...
    if (ec) luaL_errorL(L, "%s", ec.message().c_str());
    return wow::fs::push_path(L, path);
}
...
auto wow::fs::push_directory_iterator(lua_State* L, const path_t& directory, bool recursive) -> int {
...
void wow::fs::library(lua_State* L, int idx) {
